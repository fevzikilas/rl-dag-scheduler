"""HEFT and Round Robin baseline schedulers."""
...
def _eft_on_stream(
    task_node: str, stream_id: int, g: nx.DiGraph,
    task_finish_time: np.ndarray, task_proc_assigned: np.ndarray,
    proc_avail_time: np.ndarray, node_idx: dict, stream_flops: float,
) -> float:
    """Compute earliest finish time for task on stream."""
...
def round_robin(g: nx.DiGraph, gpu_memory: float = None, n_streams: int = None) -> dict:
    """Assign tasks in round-robin order to streams."""
...
def heft(g: nx.DiGraph, gpu_memory: float = None, n_streams: int = None) -> dict:
    """HEFT: rank tasks and assign to earliest-finish stream."""
    gpu_mem = gpu_memory if gpu_memory is not None else config.GPU_MEMORY_DEFAULT
    n_s = n_streams if n_streams is not None else config.N_STREAMS_DEFAULT
    stream_flops = config.GPU_FLOPS_CAP / n_s
    ...
    for nd in priority_list:
        ...
        for s in range(n_s):
            if not fits_vram:
                continue
            eft = _eft_on_stream(
                nd, s, g, task_finish_time, task_proc_assigned,
                proc_avail_time, node_idx, stream_flops,
            )
            if eft < best_eft:
                best_eft = eft
                best_stream = s
        ...
        if best_stream == -1:
            ...
            best_eft = _eft_on_stream(
                nd, best_stream, g, task_finish_time, task_proc_assigned,
                proc_avail_time, node_idx, stream_flops,
            )
